...
class CMProductionDataMonitoring:
    ...
    def TaskFilter(self, path, status):
        self.lo.log_to_file("INFO", "Executing CMProductionDataMonitoring.TaskFilter()")
        rf = ReportFileUtility(self.v_task_type)
        row_count = self.v_input_sheet.max_row
        if status == 'InProgress':
            self.lo.log_to_file("INFO", "select InProgress task from input sheet from CMProductionDataMonitoring.TaskFilter()")
            ...
            for count in range(1, row_count + 1):
                ...
                if ((self.v_input_sheet.cell(row=count, column=5).value) == "InProgress"):
                    ...
                    tto.search_process_for_CMPDM(supplier_name,retailer_name,doc_type,date, self.v_input_sheet,count)
    ...
    def execute_main(self):
        self.lo.log_to_file("INFO", "Executing CMProductionDataMonitoring.execute_main()")
        ...
        self.lo.log_to_file("INFO", "completed CMProductionDataMonitoring.execute_main()")
        self.v_driver.close()
