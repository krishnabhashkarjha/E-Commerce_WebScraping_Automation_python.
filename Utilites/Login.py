...
class Login:
    def __init__(self, v_task_type, driver, input_wb, log_file_object):
        self.driver = driver
    ...
    def login(self, v_tool):
        ...
        so = SeleniumOperations(self.task_type, self.driver, self.log_file_object)
        if v_tool in sheet_names:
            login_sheet = login_wb.get_sheet_by_name(v_tool)
        else:
            self.log_file_object.log_to_file("ERROR", "Error in Login.login(). Sheet name not found")
            return False
        ...
        if v_tool == "DC4 Prod":
            ...
            self.log_file_object.log_to_file("ERROR", "Error in Login.login(). Invalid Link")
            return False
